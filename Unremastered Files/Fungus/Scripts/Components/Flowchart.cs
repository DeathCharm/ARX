...
namespace Fungus
{
    ...
    public class Flowchart : MonoBehaviour, ISubstitutionHandler
    {
        ...
        #region ARX
        public static Flowchart Get(string name)
        {
            Flowchart[] charts = GameObject.FindObjectsOfType<Flowchart>();
            foreach (Flowchart chart in charts)
        ...
        public static Flowchart Run(string strFlowchartName, string strBlockName)
        {
            Flowchart chart = Get(strFlowchartName);
            if (chart == null)
            {
                Debug.LogError("Could not find chart with GameObject named " + strFlowchartName);
                return null;
            }

            chart.ExecuteBlock(strBlockName);
            return chart;
        }
